package btree::btree;

use strict;
use warnings;

use btree::nodo;

use constant Nodo => 'btree::nodo';

# ------------------------------------------------------------------
# Parametros del Arbol B (convencion de orden M)
#
# Definicion usada para tus clases:
# - M = maximo numero de hijos por nodo.
# - MAX_KEYS = M - 1.
# - MIN_CHILDREN = M / 2  (nodos no raiz).
# - MIN_KEYS = (M / 2) - 1 (nodos no raiz).
#
# Para esta formula exacta, M debe ser par.
# ------------------------------------------------------------------
use constant M => 4;
die "El orden M debe ser par para aplicar reglas directas M/2." if (M % 2) != 0;
use constant MAX_CHILDREN => M;
use constant MAX_KEYS => M - 1;
use constant MIN_CHILDREN => int(M / 2);
use constant MIN_KEYS => MIN_CHILDREN - 1;

# Indice de la mediana cuando un nodo lleno (3 claves) se divide.
# Para M=4: MIN_KEYS=1, por lo tanto la mediana es keys[1].
use constant SPLIT_INDEX => MIN_KEYS;

sub new {
    my ($class) = @_;

    # root: referencia a la raiz (undef mientras esta vacio)
    # size: cantidad total de claves insertadas
    my $self = {
        root => undef,
        size => 0,
    };

    bless $self, $class;
    return $self;
}

sub is_empty {
    my ($self) = @_;
    return !defined($self->{root}) ? 1 : 0;
}

sub get_size {
    my ($self) = @_;
    return $self->{size};
}

# Getters didacticos para mostrar reglas del orden elegido.
sub get_order {
    return M;
}

sub get_max_children {
    return MAX_CHILDREN;
}

sub get_max_keys {
    return MAX_KEYS;
}

sub get_min_keys_non_root {
    return MIN_KEYS;
}

# ------------------------------------------------------------------
# Insercion
#
# Estrategia usada: top-down split.
# Si al bajar encontramos un hijo lleno, lo dividimos primero y
# luego decidimos a cual de los dos hijos resultantes continuar.
# ------------------------------------------------------------------
sub insertar {
    my ($self, $key) = @_;

    # No se aceptan valores indefinidos.
    if (!defined($key)) {
        print "Advertencia: no se puede insertar valor indefinido.\n";
        return;
    }

    # En este proyecto no se permiten duplicados.
    if ($self->contiene($key)) {
        print "Advertencia: El valor '$key' ya existe en el arbol B. No se insertan duplicados.\n";
        return;
    }

    # Caso base: arbol vacio -> se crea raiz hoja con una clave.
    if ($self->is_empty()) {
        my $raiz = Nodo->new(1);
        push(@{$raiz->get_keys()}, $key);
        $self->{root} = $raiz;
        $self->{size} = 1;
        print "Insertado '$key' como RAIZ del arbol B.\n";
        return;
    }

    # Si la raiz esta llena, primero se divide para aumentar altura.
    my $r = $self->{root};
    if ($r->key_count() == MAX_KEYS) {
        print "La raiz esta llena. Se divide antes de insertar '$key'.\n";

        # Nueva raiz interna con el arbol antiguo como hijo 0.
        my $s = Nodo->new(0);
        push(@{$s->get_children()}, $r);

        # Split del hijo 0: la mediana sube a la nueva raiz.
        $self->_split_child($s, 0);
        $self->{root} = $s;
    }

    # Insercion garantizada sobre nodo no lleno.
    $self->_insert_non_full($self->{root}, $key);
    $self->{size}++;
}

# Inserta clave en un nodo que se asume no lleno.
sub _insert_non_full {
    my ($self, $node, $key) = @_;

    my $keys = $node->get_keys();
    my $i = scalar(@$keys) - 1;

    # Si es hoja, se inserta ordenadamente desplazando claves mayores.
    if ($node->is_leaf()) {
        push(@$keys, undef);
        while ($i >= 0 && $key < $keys->[$i]) {
            $keys->[$i + 1] = $keys->[$i];
            $i--;
        }
        $keys->[$i + 1] = $key;
        print "Insertado '$key' en nodo hoja.\n";
        return;
    }

    # Si es interno, se busca el indice del hijo por donde bajar.
    while ($i >= 0 && $key < $keys->[$i]) {
        $i--;
    }
    $i++;

    my $children = $node->get_children();
    my $child = $children->[$i];

    # Si el hijo destino esta lleno, primero se divide.
    if ($child->key_count() == MAX_KEYS) {
        print "Hijo en indice $i lleno. Se divide antes de bajar '$key'.\n";
        $self->_split_child($node, $i);

        # Despues del split, decidir si bajar al hijo izquierdo o derecho.
        my $new_keys = $node->get_keys();
        if ($key > $new_keys->[$i]) {
            $i++;
        }
    }

    # Continuar recursion en el hijo correcto.
    $self->_insert_non_full($node->get_children()->[$i], $key);
}

# Divide el hijo lleno parent.children[i].
sub _split_child {
    my ($self, $parent, $i) = @_;

    my $children = $parent->get_children();
    my $y = $children->[$i];               # hijo lleno original
    my $z = Nodo->new($y->is_leaf());      # nuevo hermano derecho

    my $y_keys = $y->get_keys();
    my $median = $y_keys->[SPLIT_INDEX];

    # Para M=4 y claves [k0,k1,k2]:
    # izquierda -> [k0], mediana -> k1, derecha -> [k2].
    my @left_keys  = @$y_keys[0 .. SPLIT_INDEX - 1];
    my @right_keys = @$y_keys[SPLIT_INDEX + 1 .. MAX_KEYS - 1];

    $y->set_keys(\@left_keys);
    $z->set_keys(\@right_keys);

    # Si no era hoja, repartir tambien los hijos.
    if (!$y->is_leaf()) {
        my $y_children = $y->get_children();
        my @left_children  = @$y_children[0 .. SPLIT_INDEX];
        my @right_children = @$y_children[SPLIT_INDEX + 1 .. MAX_CHILDREN - 1];
        $y->set_children(\@left_children);
        $z->set_children(\@right_children);
    }

    # Insertar mediana en el padre e insertar hermano derecho a la par.
    $parent->insert_key_at($i, $median);
    $parent->insert_child_at($i + 1, $z);

    print "Division de nodo: sube mediana '$median' al padre.\n";
}

# ------------------------------------------------------------------
# Busqueda
# ------------------------------------------------------------------
sub contiene {
    my ($self, $key) = @_;
    return defined($self->buscar($key)) ? 1 : 0;
}

sub buscar {
    my ($self, $key) = @_;

    if ($self->is_empty()) {
        print "El arbol B esta vacio.\n";
        return undef;
    }

    return $self->_buscar_recursivo($self->{root}, $key);
}

sub _buscar_recursivo {
    my ($self, $node, $key) = @_;

    my $keys = $node->get_keys();
    my $i = 0;

    # Buscar primera clave >= key.
    while ($i < scalar(@$keys) && $key > $keys->[$i]) {
        $i++;
    }

    # Si coincide, se encontro en este nodo.
    if ($i < scalar(@$keys) && $key == $keys->[$i]) {
        return $node;
    }

    # Si es hoja y no coincide, no existe.
    if ($node->is_leaf()) {
        return undef;
    }

    # Si es interno, continuar en el subarbol correspondiente.
    return $self->_buscar_recursivo($node->get_children()->[$i], $key);
}

# ------------------------------------------------------------------
# Recorridos y extremos
# ------------------------------------------------------------------
sub recorrido_inorden {
    my ($self) = @_;

    print "Recorrido INORDEN (ascendente): ";
    if ($self->is_empty()) {
        print "(arbol vacio)\n";
        return;
    }

    $self->_inorden_recursivo($self->{root});
    print "\n";
}

sub _inorden_recursivo {
    my ($self, $node) = @_;
    return unless defined($node);

    my $keys = $node->get_keys();
    my $children = $node->get_children();
    my $n = scalar(@$keys);

    # En nodo interno: hijo0, key0, hijo1, key1, ... hijoN
    for (my $i = 0; $i < $n; $i++) {
        if (!$node->is_leaf()) {
            $self->_inorden_recursivo($children->[$i]);
        }
        print $keys->[$i] . " ";
    }

    if (!$node->is_leaf()) {
        $self->_inorden_recursivo($children->[$n]);
    }
}

sub encontrar_minimo {
    my ($self) = @_;

    if ($self->is_empty()) {
        print "El arbol B esta vacio.\n";
        return undef;
    }

    # El minimo esta en la hoja mas a la izquierda.
    my $node = $self->{root};
    while (!$node->is_leaf()) {
        $node = $node->get_children()->[0];
    }
    return $node->get_keys()->[0];
}

sub encontrar_maximo {
    my ($self) = @_;

    if ($self->is_empty()) {
        print "El arbol B esta vacio.\n";
        return undef;
    }

    # El maximo esta en la hoja mas a la derecha.
    my $node = $self->{root};
    while (!$node->is_leaf()) {
        my $ult = $node->child_count() - 1;
        $node = $node->get_children()->[$ult];
    }

    my $ult_key = $node->key_count() - 1;
    return $node->get_keys()->[$ult_key];
}

# ------------------------------------------------------------------
# Impresion en consola (vista jerarquica)
# ------------------------------------------------------------------
sub imprimir_arbol {
    my ($self) = @_;

    print "\n=== Estructura del Arbol B (Orden 4) ===\n";
    if ($self->is_empty()) {
        print "(arbol vacio)\n";
        print "=========================================\n\n";
        return;
    }

    $self->_imprimir_recursivo($self->{root}, 0);
    print "=========================================\n\n";
}

sub _imprimir_recursivo {
    my ($self, $node, $nivel) = @_;
    return unless defined($node);

    my $indent = "    " x $nivel;
    my $tipo = $node->is_leaf() ? "HOJA" : "INTERNO";
    my $keys_txt = join(", ", @{$node->get_keys()});
    print $indent . "[$tipo] keys: { $keys_txt }\n";

    if (!$node->is_leaf()) {
        foreach my $child (@{$node->get_children()}) {
            $self->_imprimir_recursivo($child, $nivel + 1);
        }
    }
}

# ------------------------------------------------------------------
# Validador teorico del Arbol B
#
# Verifica:
# - Limites de claves e hijos.
# - Regla k hijos <=> k-1 claves en nodos internos.
# - Hoja a un mismo nivel.
# - Rangos de subarbol segun claves separadoras.
# - Raiz no hoja con al menos 2 hijos.
# ------------------------------------------------------------------
sub validar_propiedades {
    my ($self) = @_;

    if ($self->is_empty()) {
        return (1, "Arbol vacio (valido).");
    }

    my @errores;
    my $leaf_level = undef;

    $self->_validar_nodo(
        $self->{root},
        1,
        0,
        \$leaf_level,
        undef,
        undef,
        \@errores
    );

    if (@errores) {
        return (0, join("\n", @errores));
    }

    return (1, "El arbol cumple propiedades de Arbol B para M=" . M . ".");
}

sub _validar_nodo {
    my ($self, $node, $es_raiz, $nivel, $leaf_level_ref, $min, $max, $errores_ref) = @_;
    return unless defined($node);

    my $keys = $node->get_keys();
    my $n = scalar(@$keys);

    # Maximo de claves por nodo.
    if ($n > MAX_KEYS) {
        push(@$errores_ref, "Nodo en nivel $nivel excede maximo de claves (" . MAX_KEYS . ").");
    }

    # Minimo de claves para nodos no raiz.
    if (!$es_raiz && $n < MIN_KEYS) {
        push(@$errores_ref, "Nodo no raiz en nivel $nivel tiene menos de " . MIN_KEYS . " claves.");
    }

    # Claves estrictamente ordenadas y en rango permitido.
    for (my $i = 0; $i < $n; $i++) {
        if ($i > 0 && $keys->[$i - 1] >= $keys->[$i]) {
            push(@$errores_ref, "Claves no ordenadas estrictamente en nivel $nivel.");
        }

        if (defined($min) && $keys->[$i] <= $min) {
            push(@$errores_ref, "Clave $keys->[$i] viola limite inferior en nivel $nivel.");
        }

        if (defined($max) && $keys->[$i] >= $max) {
            push(@$errores_ref, "Clave $keys->[$i] viola limite superior en nivel $nivel.");
        }
    }

    # Si es hoja, validar nivel uniforme de todas las hojas.
    if ($node->is_leaf()) {
        if (!defined($$leaf_level_ref)) {
            $$leaf_level_ref = $nivel;
        } elsif ($$leaf_level_ref != $nivel) {
            push(@$errores_ref, "No todas las hojas estan al mismo nivel (esperado $$leaf_level_ref, encontrado $nivel).");
        }
        return;
    }

    my $children = $node->get_children();
    my $c = scalar(@$children);

    # Regla estructural basica: k claves <=> k+1 hijos.
    if ($c != $n + 1) {
        push(@$errores_ref, "Nodo interno en nivel $nivel debe tener k+1 hijos para k claves.");
    }

    # Regla especial de raiz no hoja.
    if ($es_raiz && !$node->is_leaf() && $c < 2) {
        push(@$errores_ref, "La raiz no hoja debe tener al menos 2 hijos.");
    }

    # Maximo de hijos.
    if ($c > MAX_CHILDREN) {
        push(@$errores_ref, "Nodo interno en nivel $nivel excede maximo de hijos (" . MAX_CHILDREN . ").");
    }

    # Minimo de hijos para nodos no raiz.
    if (!$es_raiz && $c < MIN_CHILDREN) {
        push(@$errores_ref, "Nodo interno no raiz en nivel $nivel tiene menos de " . MIN_CHILDREN . " hijos.");
    }

    # Validar recursivamente cada hijo con su rango correcto.
    for (my $i = 0; $i < $c; $i++) {
        my $child = $children->[$i];
        my ($child_min, $child_max) = ($min, $max);

        if ($i == 0) {
            $child_max = $keys->[0] if $n > 0;
        } elsif ($i == $c - 1) {
            $child_min = $keys->[$n - 1] if $n > 0;
        } else {
            $child_min = $keys->[$i - 1];
            $child_max = $keys->[$i];
        }

        $self->_validar_nodo(
            $child,
            0,
            $nivel + 1,
            $leaf_level_ref,
            $child_min,
            $child_max,
            $errores_ref
        );
    }
}

1;
